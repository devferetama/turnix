...
function getSelectedServiceId(pathname: string) {
  ...
  const segment = pathname.slice(`${ROUTES.services}/`.length).split("/")[0];
...
export function ServicesManagement() {
  ...
  const selectedServiceId = getSelectedServiceId(pathname);
  const isCreateRoute = pathname === ROUTES.serviceCreate;
      ...
      <PageHeader
        ...
        actions={
          <Button
            ...
            onClick={() => router.push(ROUTES.serviceCreate)}
          >
        ...
        {servicesQuery.isPending && !servicesQuery.data ? (
          ...
          <ServicesTable
            ...
            onEditService={(serviceId) =>
              router.push(ROUTES.serviceDetail(serviceId))
            }
          />
        )}
      </div>
